...
public class JDBCWorkflowStore implements WorkflowStore {
    ...
    public WorkflowEntry createEntry(String workflowName) throws StoreException {
        ...
        try {
            ...
            if(!rset.next()){
                throw new SQLException("cant insert on" + entryTable);
            }
            return new SimpleWorkflowEntry(rset.getLong(1), workflowName, WorkflowEntry.CREATED);
        } catch (SQLException e) {
    ...
    protected long createCurrentStep(Connection conn, long entryId, int wfStepId, String owner, Date startDate, Date dueDate, String status) throws SQLException {
        String sql = "INSERT INTO " + currentTable + " (" + stepEntryId + ", " + stepStepId + ", " + stepActionId + ", " + stepOwner + ", " + stepStartDate + ", " + stepDueDate + ", "
        ...
        stmt.setLong(1, entryId);
        stmt.setInt(2, wfStepId);
        stmt.setString(3, owner);
        stmt.setTimestamp(4, new Timestamp(startDate.getTime()));

        if (dueDate != null) {
            stmt.setTimestamp(5, new Timestamp(dueDate.getTime()));
        } else {
        ...
        if(!rset.next()){
            throw new SQLException("cant insert on" + currentTable);
        }
        cleanup(null, stmt, null);
    ...
    protected String getInitProperty(Map props, String strName, String strDefault) {
